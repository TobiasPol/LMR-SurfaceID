...
chains_processed = [
    chain for chain in chains if os.path.exists(f"{conf.OUTDIR}/{chain}_surface.npz")
]
...
df = df[
    df.apply(
        lambda x: (x["Ab"] in chains_processed) and (x["Ag"] in chains_processed),
        axis=1,
    )
...
if conf.CONTACT:
    ...
    print(
        f"set CONTACT =Ture or RESTRICT=True in {args.config} or provide regions column in {conf.CATALOG_PAIRS}"
    )
    exit()
...
if conf.SEARCH:
    ...
    library_within = [
        np.load(os.path.join(conf.OUTDIR, x))
        for x in df.apply(lambda x: f"{x['Ag']}_within.{x['Ab']}.npy", axis=1).to_list()
    ]
    ...
    for target in targets:
        ...
        idxs_contact = np.load(
            os.path.join(conf.OUTDIR, f"{target_}_contacts.{p2}.npy")
        )
        st_target = time()
        ...
        within = get_within(rho, list_indices, conf.neighbor_dist)
    ...
    df_hits.to_csv(
        os.path.join(conf.OUTDIR_RESULTS, f"{conf.case}_top_hits.tsv"),
        sep="\t",
        index=False,
    )
...
if conf.ALIGN:
    df_hits = pd.read_csv(
        os.path.join(conf.OUTDIR_RESULTS, f"{conf.case}_top_hits.tsv"), sep="\t"
    )
    df_hits["target_expanded_indices"] = df_hits["target_expanded_indices"].apply(
    ...
    for target in targets:
        ...
        if conf.SAVEPLY:
            ply_target = deepcopy(
                PlyData.read(os.path.join(conf.OUTDIR, f"{target_}.ply"))
            )
        ...
        for library in librarys:
            df_ = df_hits[
                (df_hits["target"] == target) & (df_hits["library epitope"] == library)
            ]
            ...
            for ie, entry in df_.iterrows():
                ...
                with torch.set_grad_enabled(False):
                    ...
                    for i1 in range(0, n2, mini_bs):
                        ...
                        fs_ = torch.tensor(fs[i1:i2]).to(device).view(1, i2 - i1, DIM)
                ...
                try:
                    ...
                    fs0 = o_target[idxs_target]
                    _, _, score3 = get_score2(xyz0, fs0, n0, xyz_patch, fs, n, SIG)
                except:
                    n_score, score2, score3 = 0.0, 100.0, 1000.0
                ...
                logger.info(
                    f" Time for aligning {library} to {target}: {time()-st:.2f}"
                )
                if conf.SAVEPLY:
                    ...
                    els = []
                    for tag, iis in zip(
                        ["hit_patch", "contact_patch"], [idxs_library, idxs_library0]
                    ):
                        ...
                        vertex_patch = np.asarray(
                            [tuple(q) for q in xyz_patch_],
                    ...
                    fname = os.path.join(
                        conf.OUTDIR_RESULTS,
                        f"{id_counter}_{entry['library epitope']}_{target}.ply",
                    )
                    ...
                    fname = os.path.join(
                        conf.OUTDIR_RESULTS,
                        f"{id_counter}_{entry['library epitope']}_{target}.pdb",
                    )
                    io = PDBIO()
                    io.set_structure(structure)
                    io.save(fname)
        logger.info(
            f" Total time for aligning hits to {target}: {time()-st_target:.2f}"
        )
        if conf.SAVEPLY:
            out_target_fname = os.path.join(conf.OUTDIR_RESULTS, f"{target}_ref.ply")
            ply_target.write(out_target_fname)
        df_hits.to_csv(
            os.path.join(conf.OUTDIR_RESULTS, f"{conf.case}_top_hits_aligned.tsv"),
            sep="\t",
            index=False,
        )
...
if conf.HEATMAP:
    df_hits = pd.read_csv(
        os.path.join(conf.OUTDIR_RESULTS, f"{conf.case}_top_hits_aligned.tsv"), sep="\t"
    )
    cols = [
        "library epitope",
        "aln1_score2",
    ...
    df_hits.rename(columns={c: cn for c, cn in zip(cols, cols_new)}, inplace=True)
    fname = os.path.join(conf.OUTDIR_RESULTS, f"{conf.case}_top_hits_aligned.tsv")
    df_hits.to_csv(fname, sep="\t", index=False)

    iselect = (
        df_hits.apply(lambda x: x["target"] not in x["library_epitope"], axis=1)
        & (df_hits["spatial_sim"] < 0.6)
        & (df_hits["spatial_sim"] >= 0.5)
    )
    print(
        df_hits[iselect]
        .sort_values(
            ["target", "library_epitope", "frac_library_hits"], ascending=False
        )[["id"] + cols_new]
        .drop_duplicates()
        .head(50)
    )
    rows = df["Ag"].to_numpy()
    cols = df.apply(
        lambda x: f'{x["Ag"]}.{x["Ab"]}', axis=1
    ).to_numpy()  # sorted(df_hits["target epitope"].drop_duplicates().tolist())
    if conf.RESTRICT:
        rows = cols
    ...
    for rank in [
        ...
        for i, r in enumerate(rows):
            for j, c in enumerate(cols):
                df_ = df_hits[
                    (df_hits["target"] == r) & (df_hits["library_epitope"] == c)
                ]
                if len(df_):
                    q = func(df_[rank])
            ...
            grid_kws = {"height_ratios": (0.05, 0.90), "hspace": 0.3}
