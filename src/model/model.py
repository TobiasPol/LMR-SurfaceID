...
class Model(nn.Module):
    def __init__(
        ...
        if share_soft_grid_across_channel:
            ...
            if add_center_pixel:
                a = np.full((1, 1, 1, 1, 1), drho / 2.0, dtype=np.float32)
                self.sig_rho_center = nn.parameter.Parameter(torch.tensor(a))
        else:
            # [B, 1, Nf, Ng, Np]
            thetas = torch.cat(
                [thetas.reshape(1, 1, 1, self.ngrid, 1)] * num_filters, dim=1
            )
            rhos = torch.cat(
                [rhos.reshape(1, 1, 1, self.ngrid, 1)] * num_filters, dim=1
            )
            thetas = thetas.reshape(1, 1, num_filters, self.ngrid, 1)
            rhos = rhos.reshape(1, 1, num_filters, self.ngrid, 1)
            if add_center_pixel:
                a = np.full((1, 1, num_filters, 1, 1), drho / 2.0, dtype=np.float32)
                self.sig_rho_center = nn.parameter.Parameter(torch.tensor(a))
        ...
        sigs_rho = torch.tensor(np.full(rhos.size(), drho / 2.0, dtype=np.float32))
        ...
        if conv_type == "sep":
            ...
            Ws = [
                torch.empty(num_filters, self.ngrid, dtype=torch.float32)
                for _ in range(self.num_in)
            ]
            for W in Ws:
                nn.init.xavier_normal_(W)
            W = torch.cat([W.unsqueeze(-1) for W in Ws], dim=-1).reshape(
                1, 1, num_filters, self.ngrid, self.num_in
            )
            if add_center_pixel:
                Ws_center = [
                    torch.empty(num_filters, 1, dtype=torch.float32)
                    for _ in range(self.num_in)
                ]
                for W_center in Ws_center:
                    nn.init.xavier_normal_(W_center)
                W_center = torch.cat(
                    [W_center.unsqueeze(-1) for W_center in Ws_center], dim=-1
                ).reshape(1, 1, num_filters, 1, self.num_in)
    ...
    def forward(self, x, rho, theta, mask, pos=None, neg=None, calc_loss=True):
        ...
        if self.add_center_pixel:
            ...
            arg_center = torch.square(
                rho / torch.clamp(self.sig_rho_center, min=self.min_sig)
            )
            ...
            # map to grid
            z_center = (weight_center.unsqueeze(-1) * x).sum(
                dim=-2
            )  # sum over points; out [B, Nrot=1, Nf=1, Ng=1, num_in]
            h_center = (z_center * self.W_center).sum(dim=-2)  # [B, 1, 1, num_in]

        arg0 = torch.square(
            (rho - self.rhos) / torch.clamp(self.sigs_rho, min=self.min_sig)
        )
        theta_ = torch.remainder(theta + self.dthetas, 2 * np.pi)
        arg1 = torch.square(
            (theta_ - self.thetas) / torch.clamp(self.sigs_theta, min=self.min_sig)
        )
        arg = arg0 + arg1
        ...
        # map features
        z = (weight.unsqueeze(-1) * x).sum(
            dim=-2
        )  # sum over points; out [B, Nrot, NF, Ng, num_in]
        ...
        if self.conv_type == "sep":
            o = h.view(
                B, self.nrotation, self.num_filters * self.num_in
            )  # .contiguous() # [B, NF * num_in]
        else:
    ...
    def get_dist(self, o, pos_mask, neg_mask):
        ...
        d_pos = d[pos_mask == 1.0]
        d_neg = d[neg_mask == 1.0]
        d_pos_loss = torch.clamp(d_pos, min=0.0)
        d_neg_loss = torch.clamp(self.neg_margin - d_neg, min=0.0)
    ...
    def get_dist_light(self, o, pos_mask, neg_mask, bs=128):
        ...
        d_pos = d[pos_mask == 1.0]
        d_neg = d[neg_mask == 1.0]
        d_pos_loss = np.maximum(d_pos, 0.0)
        d_neg_loss = np.maximum(self.neg_margin - d_neg, 0.0)
...
def sample_patches(p, nsample_per_mol, data_type, rmax):
    ...
    x = np.concatenate(
        [p.x[p.list_indices[idx]].reshape(1, 200, 5) for idx in idxs_patch]
    )

    return (x, p.rho[idxs_patch], p.theta[idxs_patch], p.mask[idxs_patch], pos, neg)
...
if __name__ == "__main__":
    pdbs_train = pd.read_csv("20201107_SAbDab_masif_train_pdbs.tsv", sep="\t")[
        "pdb"
    ].to_numpy()
    pdbs_test = pd.read_csv("20201107_SAbDab_masif_test_pdbs.tsv", sep="\t")[
        "pdb"
    ].to_numpy()
    pdbs_val = pd.read_csv("20201107_SAbDab_masif_val_pdbs.tsv", sep="\t")[
        "pdb"
    ].to_numpy()
    ...
    for prefix, params, train_params in expts:
        ...
        if prefix in expts_train:
            ...
            for ckpt in range(train_params["max_ckpts"]):
                ...
                train_loss_tmp = 0
                ps = np.random.choice(
                    pdbs_train, train_params["nmol_per_ckpt"], replace=False
                )
                fs = [os.path.join(OUTDIR, p) + "_surface.npz" for p in ps]
                # # parallel
                ...
                for j in range(train_params["nbatch_ckpt"]):
                    ...
                    x_, rho_, theta_, mask_, pos_, neg_ = get_batch(
                        device,
                        train_params["nsample_per_mol"],
                        train_params["data"],
                        train_params["num_mol"],
                        mols,
                        train_params["rmax"],
                        params["rho_max"],
                    )
                    ...
                    nneg += neg_.sum().item()
                    o, loss, _, _ = model(
                        x_, rho_, theta_, mask_, pos_, neg_, calc_loss=True
                    )
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(
                        model.parameters(), train_params["grad_norm_clip"]
                    )
                ...
                print(
                    f"{ckpt} avg pos: {npos / train_params['nbatch_ckpt'] / train_params['num_mol']}"
                )
                print(
                    f"{ckpt} avg neg: {nneg / train_params['nbatch_ckpt'] / train_params['num_mol']}"
                )
                ...
                plt.savefig(
                    os.path.join(savedir, f"loss_{prefix}.png"),
                    dpi=200,
                    bbox_inches="tight",
                )
                plt.close()
        ...
        ax.set_ylabel("Count")
        plt.savefig(
            os.path.join(savedir, f"descr_dist_count_{prefix}.png"),
            dpi=200,
            bbox_inches="tight",
        )
        plt.close()
        ...
        ax.set_title(prefix)
        ax.hist(
            d_pos, bins=bins, alpha=0.5, density=True, label=f"self-neighbor [0.5, 1.5]"
        )
        ...
        plt.savefig(
            os.path.join(savedir, f"descr_dist_density_{prefix}.png"),
            dpi=200,
            bbox_inches="tight",
        )
        plt.close()
