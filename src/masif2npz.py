...
def save_masif_surface(pdb, chain):
    ...
    list_indices = np.load(
        os.path.join(precomp_dir_pdb_chain, "p1_list_indices.npy"), allow_pickle=True
    )
    list_indices = [
        np.concatenate([x, np.ones(200 - len(x), dtype=int) * -1]).reshape(1, 200)
        if len(x) < 200
        else np.asarray(x).reshape(1, 200)
