...
def compute_aux_contact(
    ...
    if mode == "iface":
        logger.info(p1, f"{len(ii1) / len(if1):.3f}", len(ii1), len(if1), len(x1))
        logger.info(p2, f"{len(ii2) / len(if2):.3f}", len(ii2), len(if2), len(x2[0]))
    # assert False
